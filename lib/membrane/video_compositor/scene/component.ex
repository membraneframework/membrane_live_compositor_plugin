defmodule Membrane.VideoCompositor.Scene.Component do
  ...
  and can inject local state into the target's state (`inject_into_target_state/2`).
  Can use NIFs
  """
  ...
  """
  @callback handle_init(options_t()) ::
  ...
  """
  @callback inject_into_target_state(target_state_t(), state_t()) ::
              {:ok, target_state_t()} | {:error, error_t()}

  @callback handle_update(target_state_t(), state_t(), time_t()) ::

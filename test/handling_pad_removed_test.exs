defmodule Membrane.VideoCompositor.HandlingPadRemovedTest do
  ...
  test "when a video that did not receive start of stream gets removed it should unblock others" do
    ...
    assert {[stream_format: {:output, ^stream_format}], state} =
             CompositorElement.handle_playing(%{}, state)

    pad1 = {Membrane.Pad, :input, 1}
    pad2 = {Membrane.Pad, :input, 2}

    assert {[], state} =
             CompositorElement.handle_pad_added(pad1, %{options: pad_added_opts}, state)

    assert {[], state} = CompositorElement.handle_stream_format(pad1, stream_format, %{}, state)
    assert {[], state} = CompositorElement.handle_start_of_stream(pad1, %{}, state)

    assert {[], state} =
             CompositorElement.handle_pad_added(pad2, %{options: pad_added_opts}, state)

    assert {[], state} = CompositorElement.handle_stream_format(pad2, stream_format, %{}, state)
    assert {[], state} = CompositorElement.handle_start_of_stream(pad2, %{}, state)

    second = Membrane.Time.second()
    buffer1 = %Buffer{payload: <<0::size(48)>>, pts: 0}
    assert {[], state} = CompositorElement.handle_process(pad1, buffer1, %{}, state)

    buffer2 = %Buffer{payload: <<0::size(48)>>, pts: second}
    assert {[], state} = CompositorElement.handle_process(pad1, buffer2, %{}, state)
    ...
    assert {[
            ...
            ], _state} = CompositorElement.handle_pad_removed(pad2, %{pads: pads}, state)
  end
end

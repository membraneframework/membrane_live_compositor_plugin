defmodule Membrane.VideoCompositor.Benchmark.MergeFrames do
  ...
  def benchmark() do
    ...
    {:ok, ffmpeg_internal_state_720p} = Membrane.VideoCompositor.Implementation.FFmpeg.init(caps_720p)
    {:ok, ffmpeg_internal_state_1080p} = Membrane.VideoCompositor.Implementation.FFmpeg.init(caps_1080p)
    {:ok, ffmpeg_internal_state_4k} = Membrane.VideoCompositor.Implementation.FFmpeg.init(caps_4k)

    {:ok, opengl_cpp_internal_state_720p} = Membrane.VideoCompositor.Implementation.OpenGL.Cpp.init(caps_720p)
    {:ok, opengl_cpp_internal_state_1080p} = Membrane.VideoCompositor.Implementation.OpenGL.Cpp.init(caps_1080p)
    {:ok, opengl_cpp_internal_state_4k} = Membrane.VideoCompositor.Implementation.OpenGL.Cpp.init(caps_4k)

    {:ok, opengl_rust_internal_state_720p} = Membrane.VideoCompositor.Implementation.OpenGL.Rust.init(caps_720p)
    {:ok, opengl_rust_internal_state_1080p} = Membrane.VideoCompositor.Implementation.OpenGL.Rust.init(caps_1080p)
    {:ok, opengl_rust_internal_state_4k} = Membrane.VideoCompositor.Implementation.OpenGL.Rust.init(caps_4k)
    ...
    Benchee.run(
      %{
        "FFmpeg - Merge two frames to one" =>
          fn {frames, internal_states} -> Membrane.VideoCompositor.Implementation.FFmpeg.merge_frames(frames, internal_states.ffmpeg) end,
        "OpenGL C++ - Merge two frames to one" =>
          fn {frames, internal_states} -> Membrane.VideoCompositor.Implementation.OpenGL.Cpp.merge_frames(frames, internal_states.opengl_cpp) end,
        "OpenGL Rust - Merge two frames to one" =>
          fn {frames, internal_states} -> Membrane.VideoCompositor.Implementation.OpenGL.Rust.merge_frames(frames, internal_states.opengl_rust) end,

...
impl YUVPlanarTexture {
    ...
    pub fn new(width: usize, height: usize) -> Result<Self, CompositorError> {
        ...
        unsafe { gl!(gl::GenTextures(3, texture_ids.as_mut_ptr()))? }

        for id in texture_ids {
            unsafe {
                gl!(gl::BindTexture(gl::TEXTURE_2D, id))?;
                gl!(gl::TexParameteri(
                    gl::TEXTURE_2D,
                    gl::TEXTURE_WRAP_S,
                    gl::REPEAT as i32
    ...
    pub fn bind(&self) -> Result<(), CompositorError> {
        for (i, &id) in self.texture_ids.iter().enumerate() {
            unsafe {
                gl!(gl::ActiveTexture(gl::TEXTURE0 + i as u32))?;
                gl!(gl::BindTexture(gl::TEXTURE_2D, id))?;
            }
        }
    ...
    pub fn load_frame(&self, data: &[u8]) -> Result<(), CompositorError> {
        ...
        unsafe {
            ...
            gl!(gl::ActiveTexture(gl::TEXTURE2))?;
            gl!(gl::TexImage2D(
                gl::TEXTURE_2D,
                0,
                gl::R8 as i32,
...
impl Drop for YUVPlanarTexture {
    fn drop(&mut self) {
        unsafe { gl!(gl::DeleteTextures(3, self.texture_ids.as_ptr())).unwrap() }
    }
}

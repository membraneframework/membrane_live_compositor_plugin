...
void YUVTexture::load(const char *data) const {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, m_width / 2, m_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, data + pixel_amount);

    glActiveTexture(GL_TEXTURE2);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R8, m_width / 2, m_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, data + pixel_amount + pixel_amount / 4);
}

...
BasicFBO::BasicFBO(unsigned int width, unsigned int height, GLenum internal_format, GLenum format, GLenum type)
    : m_width(width)
    , m_height(height)
    , m_id(0)
    , m_renderbuffer_id(0)
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_renderbuffer_id);
...
GLuint BasicFBO::id() const{
    return m_id;
}

GLuint BasicFBO::renderbuffer_id() const{
    return m_renderbuffer_id;
}
